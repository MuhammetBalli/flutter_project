...
class _PopularTvShowsState extends State<PopularTvShows> {
  ...
  void _onScroll() {
    ...
    if (_scrollController.position.pixels >=
        ...
        !tvShowProvider.isPopularTvShowLoading) {
      tvShowProvider.fetchPopularTvShows();
    }
  }
    ...
    return Scaffold(
      body: Consumer<TvShowProvider>(
        builder: (context, tvShowProvider, child) {
          if (tvShowProvider.isPopularTvShowLoading &&
              tvShowProvider.tvShowsPopular.isEmpty) {
            return const Center(child: CircularProgressIndicator());
          } else if (tvShowProvider.tvShowsPopular.isEmpty) {
              ...
              children: [
                ...
                Container(
                  ...
                  child: ListView.builder(
                    ...
                    itemCount: tvShowProvider.tvShowsPopular.length +
                        (tvShowProvider.isPopularTvShowLoading ? 1 : 0),
                    itemBuilder: (context, index) {
                      if (index == tvShowProvider.tvShowsPopular.length) {
                        return const Center(child: CircularProgressIndicator());
                      }
                      final tvShow = tvShowProvider.tvShowsPopular[index];
